...
uint256 const hashGenesisBlock("0x00000f111ba75f348716a6431e0a7ca215becbbf2cc003d0c0d39d531dc44311");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Doomcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Doomcoin Signed Message:\n";
...
{
    // Doomcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight == 1) { nSubsidy = 22000 * COIN; } else
    if (nHeight <= 1000 ) { nSubsidy = 50 * COIN; } else
    if (nHeight <= 5000 ) { nSubsidy = 25 * COIN; } else
    if (nHeight <= 10000 ) { nSubsidy = 13 * COIN; } else
    if (nHeight <= 15000 ) { nSubsidy = 7 * COIN; } else
    if (nHeight <= 385600 ) { nSubsidy = 5 * COIN; }

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 30 * 60; // 30 minutes
static const int64 nTargetSpacing = 4 * 60; // 4 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Doomcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Doomcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        const char* pszTimestamp = "CNN 28/07/2014 U.N. Security Council demands humanitarian cease-fire in Gaza";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f686372bbfaf1b07379eab4743512c3a08dba687512a31e456fa8a4ae1d58ce5651b9eb31048dea0769996f8a11cb0c52fc9cc5caaf7eb5157282e21dccb290c") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1406538505;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 4219456;
        ...
        assert(block.hashMerkleRoot == uint256("0xe30bfaf630f25d6386d00fb0fc8bd444e195d5097daab53667fb647a777843a7"));
...
unsigned char pchMessageStart[4] = { 0xc4, 0xdf, 0xe9, 0x99 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DoomcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DoomcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DoomcoinMiner(CWallet *pwallet)
{
    printf("DoomcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("doomcoin-miner");
    ...
    try { loop {
        ...
        printf("Running DoomcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DoomcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DoomcoinMiner, pwallet));
}
