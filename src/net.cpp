...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Doomcoin " + FormatFullVersion();
...
static const char *strMainNetOnionSeed[][1] = {
    {"mlyz4wmxpfrcli4k.onion"},
    {"lbqjpuiqowr7mw5z.onion"},
    {NULL}
};
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Doomcoin is probably already running."), addrBind.ToString().c_str());
        else
